...
func main() {
	...
	for i := 0; i < 3; i++ {
		...
		entry := &changelogs.SendChangeLogRequest{
			Entry: &changelogs.ChangeLogEntry{
				...
				ExternalName:      fmt.Sprintf("object-%d", i),
		...
		if err != nil {
			log.Printf("failed to send change log entry: %+v", err)
		}

		resp := &changelogs.SendChangeLogResponse{}
	...
	if err != nil {
		log.Fatalf("failed to close change log stream: %+v", err)
	}
}
